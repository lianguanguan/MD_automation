...
def travel_cases(automation_cases):
    test_cases = ['source1,idle,idle,hold,answer']
    count = 0
    success_num = 0
    failed_case_1st = []
    delay_send_hs_key = 0.2
    case_interval_delay = 1.2
    ...
    for case in automation_cases:
        ...
        time.sleep(case_interval_delay)
...
def get_response_on_aux(expected_event):
    aux_response_delay = 5
    if expected_event is 'answer':
        try:
            child_aux_server.expect('answer', timeout=aux_response_delay)
        except pexpect.TIMEOUT:
            return False

    elif expected_event is 'hold':
        try:
            child_aux_server.expect('hold', timeout=aux_response_delay)
        except pexpect.TIMEOUT:
            return False

    elif expected_event is 'reject':
        try:
            child_aux_server.expect('reject', timeout=aux_response_delay)
        except pexpect.TIMEOUT:
            return False

    elif expected_event is 'resume':
        try:
            child_aux_server.expect('resume', timeout=aux_response_delay)
        except pexpect.TIMEOUT:
            return False
...
def get_response_on_usb2(expected_event):
    ...
    timer = threading.Timer(5, rx_time_out_usb2)
    timer.start()
...
def get_response_on_bt(expected_event):
    ...
    if expected_event is 'answer':
        try:
            child_bt.expect('BT_HFG_EVENT_ANSWER', timeout=bt_response_delay)
        except pexpect.TIMEOUT:
            return False

    elif expected_event is 'hold':
        try:
            child_bt.expect('BT_HFG_EVENT_HOLD_AND_ACCEPT', timeout=bt_response_delay)
        except pexpect.TIMEOUT:
            return False

    elif expected_event is 'reject':
        try:
            child_bt.expect('BT_HFG_EVENT_HANGUP', timeout=bt_response_delay)
        except pexpect.TIMEOUT:
            return False
